...
public class EduAIBackend {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/api/getUser", new GetUserHandler());
   ...
   private static void sendResponse(HttpExchange exchange, int code, String response) throws IOException {
    ...
    // ===== Add this handler (paste after GetDocumentsHandler) =====
static class GetUserHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (query == null || !query.contains("email=")) {
            sendResponse(exchange, 400, "{\"error\":\"Email required\"}");
            return;
        }

        // get everything after "email=" (handles simple case)
        String email = query.split("email=")[1];
        // if there are other params, take until '&'
        if (email.contains("&")) email = email.split("&")[0];

        // decode URL-encoded email (optional but safe)
